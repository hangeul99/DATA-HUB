...
/**
 * 온보딩 페이지
 * - 구글 등 소셜 로그인 직후 organization 미입력 사용자에게 표시
 * - proxy.ts에서 organization = null 이면 이 페이지로 리다이렉트
 * - 소속기관 입력 후 profiles 테이블 업데이트
 */
export default function OnboardingPage() {
  ...
  // 이미 온보딩 완료한 사용자는 홈으로 이동 (뒤로가기 방지)
  useEffect(() => {
    ...
    supabase.auth.getUser().then(async ({ data: { user } }) => {
      ...
      const { data: profile } = await supabase
        ...
        .eq("id", user.id)
        .single();
      if (profile?.organization) router.replace("/");
    });
  }, [router]);

  // 소속기관을 profiles 테이블에 저장
