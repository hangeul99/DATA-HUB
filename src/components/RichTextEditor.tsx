...
const FONTS = [
  { label: "Í∏∞Î≥∏Ï≤?,  value: "Arial, sans-serif" },
  { label: "Î™ÖÏ°∞Ï≤?,  value: "Georgia, serif" },
  { label: "Í≥†Ï†ï??,  value: "'Courier New', monospace" },
  { label: "?êÍ???,  value: "cursive" },
];

export default function RichTextEditor({ value, onChange, placeholder = "?¥Ïö©???ÖÎ†•?òÏÑ∏??" }: Props) {
  ...
  return (
    <div className="rounded-xl border border-neutral-200 overflow-hidden">
      ...
      <div className="flex items-center flex-wrap gap-1 px-3 py-2 bg-neutral-50 border-b border-neutral-200">
        ...
        >
          <option value="" disabled>Í∏Ä?êÏ≤¥</option>
        ...
        <select
          ...
          className="text-xs border border-neutral-200 rounded px-1.5 py-1 bg-white focus:outline-none cursor-pointer"
        >
          <option value="" disabled>?¨Í∏∞</option>
          <option value="10px">??/option>
          <option value="14px">Ï§?/option>
          <option value="20px">?Ä</option>
          <option value="28px">?πÎ?</option>
        </select>

        <span className="w-px h-4 bg-neutral-300" />

        <Btn title="ÍµµÍ≤å" onClick={() => exec("bold")} className="font-bold">B</Btn>
        <Btn title="Í∏∞Ïö∏Í∏? onClick={() => exec("italic")} className="italic">I</Btn>
        <Btn title="Î∞ëÏ§Ñ" onClick={() => exec("underline")} className="underline">U</Btn>
        <Btn title="Ï∑®ÏÜå?? onClick={() => exec("strikeThrough")} className="line-through">S</Btn>

        <span className="w-px h-4 bg-neutral-300" />

        <label title="Í∏Ä???? className="inline-flex items-center gap-0.5 cursor-pointer px-2 py-1 text-xs hover:bg-neutral-200 rounded transition-colors select-none">
          A
          <input
            type="color"
        ...
        <Btn title="Î≤àÌò∏ Î™©Î°ù" onClick={() => exec("insertOrderedList")}>1.</Btn>
        <Btn title="Í∏ÄÎ®∏Î¶¨ Î™©Î°ù" onClick={() => exec("insertUnorderedList")}>??/Btn>

        <span className="w-px h-4 bg-neutral-300" />

        <Btn title="?ºÏ™Ω ?ïÎ†¨" onClick={() => exec("justifyLeft")}>
        ...
        <Btn title="Í∞Ä?¥Îç∞ ?ïÎ†¨" onClick={() => exec("justifyCenter")}>
          <svg width="13" height="13" fill="currentColor" viewBox="0 0 13 13">
            <rect x="0"   y="1"   width="13" height="2" rx="1"/>
        ...
        <Btn title="?§Î•∏Ï™??ïÎ†¨" onClick={() => exec("justifyRight")}>
          <svg width="13" height="13" fill="currentColor" viewBox="0 0 13 13">
            <rect x="0" y="1"   width="13" height="2" rx="1"/>
        ...
        <Btn title="?úÏãù ?úÍ±∞" onClick={() => exec("removeFormat")}>??/Btn>
      </div>
